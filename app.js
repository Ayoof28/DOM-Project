...
const slide = document.querySelectorAll(".slide"); // To handle all slides
...
const calculateSlideWidth = () => {
  return slide[1].clientWidth; // Assuming all slides have the same width
};

const handleSlide = (index) => {
  const totalSlides = slide.length - 1; // Total number of slides
...
btnClear.addEventListener("click",() => {
  inputs.forEach( itemName => itemName.value = "");
} ); 
btnClear.addEventListener("click",() => {
  inputs.forEach( quantity => quantity.value = "");
} ); 
btnClear.addEventListener("click",() => {
  inputs.forEach( price => price.value = "");
} ); 
